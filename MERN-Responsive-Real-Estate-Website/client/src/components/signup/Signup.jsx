...
import { AiFillEye,AiTwotoneEyeInvisible,AiOutlinePlus,AiFillLock, AiOutlineUnlock,AiOutlineMail} from 'react-icons/ai'
...
const Signup = () => {
  ...
  const handleState = (e) => {
    ...
    if (password.length < 4) {
      setMistake("Password:Must be minimum 4 characters include one UPPERCASE, lowercase, number and special character: @$! % * ? &");
      setValid(false)
      return;
    }
    else {
  ...
  return (
    <div className={classes.container}>
      ...
      <div className={classes.photos}><img src='./signUp.png' alt=''/></div>
      <div className={classes.signUpHandle}>
        ...
        <form onSubmit={handleRegister}>
          ...
          <span className={classes.iconsSignUPOne}><BsPencilFill size="20px" /></span>
         </div>
          ...
          <span className={classes.iconsSignUPTwo}><AiOutlineMail /></span>
           </div>
          
          <div className={classes.toShow}>
            <div><AiFillLock/></div>
          <input type={showPassword ? 'text' : 'password'} name="password"  placeholder='Password..' onChange={handleState} />
          <span  className={classes.visibility} style={{color:"#117964;"}}> {showPassword ? (
        < AiTwotoneEyeInvisible color="green" onClick={togglePasswordVisibility} />
      ) : (
        < AiFillEye onClick={togglePasswordVisibility} />
          ...
          <div className={classes.strength}>
            <PasswordStrengthMeter password={password} actions={dataHandler}/>
          </div>
       {isStrength === 'Hurrah-you-did -it' && valid===true && 
            <button  className={classes.buttonSignup} type="submit"> Register </button>
        }
        </form>
        ...
        {emptyFields && (
         ...
         <div className={classes.para}> <span>Already have an account? <Link to='/signIn'>Login</Link></span></div></div>
              

      </div>
      {isMistake !== null && isMistake && (
              <p className={isMistake !== null && valid===true && isMistake && isMistake ? classes.valid: classes.erode  }> {isMistake}</p>
            )}
